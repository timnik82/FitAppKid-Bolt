/*
# Create Child Profile Function

This migration creates a SECURITY DEFINER function to handle child profile creation
and linking to parent accounts. This approach bypasses RLS policy conflicts by
executing the operations with elevated database privileges.

## Changes Made
1. Created `create_child_profile_and_link` function
2. Function handles complete child profile creation workflow
...
- Creates child profile with proper COPPA compliance settings
- Links child to parent via parent_child_relationships
- Initializes child's user progress
- Returns complete child profile data
*/
