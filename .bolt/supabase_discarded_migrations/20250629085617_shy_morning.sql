/*
# Integration Tests for Children's Fitness App Database

## Overview
This migration runs comprehensive integration tests to verify end-to-end functionality
of the children's fitness app database schema. Tests are designed to work within
the existing foreign key constraints.

## Tests Performed
1. Database structure validation
2. Exercise data integrity verification
3. Adventure and reward system validation
4. RLS policy verification
5. Data relationship integrity
6. COPPA compliance features
7. Performance and indexing validation

## Note
These tests validate the schema without creating actual user records,
as that requires Supabase Auth integration in the application layer.
*/
...
BEGIN
    ...
    IF index_count < 10 THEN
        RAISE EXCEPTION 'Insufficient indexes found. Expected at least 10, found %', index_count;
    END IF;
    ...
    IF constraint_count < 15 THEN
        RAISE EXCEPTION 'Insufficient foreign key constraints. Expected at least 15, found %', constraint_count;
    END IF;
...
BEGIN
    ...
    IF structured_exercise_count = 0 THEN
        RAISE EXCEPTION 'No exercises have structured data (sets/reps/duration)';
    END IF;
    ...
    IF muscle_group_count < 5 THEN
        RAISE EXCEPTION 'Insufficient muscle groups. Expected at least 5, found %', muscle_group_count;
    END IF;
    ...
    IF equipment_count < 3 THEN
        RAISE EXCEPTION 'Insufficient equipment types. Expected at least 3, found %', equipment_count;
    END IF;
...
BEGIN
    ...
    IF adventure_count = 0 THEN
        RAISE EXCEPTION 'No active adventures found';
    END IF;
    ...
    IF adventure_path_count = 0 THEN
        RAISE EXCEPTION 'No active adventure paths found';
    END IF;
    ...
    IF reward_count = 0 THEN
        RAISE EXCEPTION 'No active rewards found';
    END IF;
    
    -- Verify adventure-exercise relationships
    ...
    IF adventure_exercise_count = 0 THEN
        RAISE EXCEPTION 'No adventure-exercise relationships found';
    END IF;
    
    -- Verify path-exercise relationships
    ...
    IF path_exercise_count = 0 THEN
        RAISE EXCEPTION 'No path-exercise relationships found';
    END IF;
    
    -- Test adventure data structure
    SELECT 
        a.title,
        a.difficulty_level,
        a.total_exercises,
    ...
    -- Test adventure path data structure
    SELECT 
        ap.title,
        ap.theme,
        ap.difficulty_level,
...
BEGIN
    ...
    IF progression_levels < 2 THEN
        RAISE EXCEPTION 'Insufficient progression levels. Expected at least 2, found %', progression_levels;
    END IF;
...
BEGIN
    ...
    IF policy_count < 20 THEN
        RAISE EXCEPTION 'Insufficient RLS policies. Expected at least 20, found %', policy_count;
    END IF;
...
BEGIN
    ...
    IF check_constraints < 10 THEN
        RAISE EXCEPTION 'Insufficient check constraints. Expected at least 10, found %', check_constraints;
    END IF;
    
    IF unique_constraints < 15 THEN
        RAISE EXCEPTION 'Insufficient unique constraints. Expected at least 15, found %', unique_constraints;
    END IF;
    
    IF foreign_key_constraints < 15 THEN
        RAISE EXCEPTION 'Insufficient foreign key constraints. Expected at least 15, found %', foreign_key_constraints;
    END IF;
...
BEGIN
    ...
    IF performance_indexes < 10 THEN
        RAISE EXCEPTION 'Insufficient performance indexes. Expected at least 10, found %', performance_indexes;
    END IF;
...
BEGIN
    ...
    IF function_count < 3 THEN
        RAISE EXCEPTION 'Insufficient custom functions. Expected at least 3, found %', function_count;
    END IF;
    
    IF trigger_count < 3 THEN
        RAISE EXCEPTION 'Insufficient triggers. Expected at least 3, found %', trigger_count;
    END IF;
