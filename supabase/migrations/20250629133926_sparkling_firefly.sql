...
This migration restructures the profiles table to support COPPA-compliant child accounts
that don't require authentication while maintaining secure parent oversight.

## Changes Made
1. Add profile_id as new primary key for profiles table
2. Make user_id nullable (only parents have auth accounts)
3. Update all foreign key references to use profile_id
4. Create new RLS policies for the restructured schema
5. Support child profiles without auth.users entries

## COPPA Compliance
- Parent profiles: user_id = auth.uid(), is_child = false
- Child profiles: user_id = NULL, is_child = true
- Parent-child relationships use profile_id for linking
*/
...
-- Step 3: Drop ALL existing policies completely to avoid dependency issues
DO $$
DECLARE
    pol record;
BEGIN
...
-- Step 5: Add new profile_id column as the future primary key
...
-- Step 7: Make profile_id NOT NULL
ALTER TABLE profiles ALTER COLUMN profile_id SET NOT NULL;

-- Step 8: Drop the old primary key constraint and create new one
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_pkey;
ALTER TABLE profiles ADD CONSTRAINT profiles_pkey PRIMARY KEY (profile_id);

-- Step 9: Rename id column to user_id and make it nullable
ALTER TABLE profiles RENAME COLUMN id TO user_id;
ALTER TABLE profiles ALTER COLUMN user_id DROP NOT NULL;

-- Step 10: Add foreign key constraint for user_id to auth.users (allows NULL for children)
-- We'll add this constraint, but it allows NULL values for child profiles
ALTER TABLE profiles ADD CONSTRAINT profiles_user_id_fkey 
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- Step 11: Create a temporary mapping table to help with data migration
CREATE TEMP TABLE profile_id_mapping AS
SELECT user_id AS old_user_id, profile_id AS new_profile_id
FROM profiles;

-- Step 12: Update parent_child_relationships table to use profile_id
-- Add new columns
ALTER TABLE parent_child_relationships ADD COLUMN IF NOT EXISTS new_parent_id uuid;
ALTER TABLE parent_child_relationships ADD COLUMN IF NOT EXISTS new_child_id uuid;

-- Migrate existing data by matching old user_id to new profile_id
UPDATE parent_child_relationships 
SET new_parent_id = m.new_profile_id
FROM profile_id_mapping m
WHERE parent_child_relationships.parent_id = m.old_user_id;

UPDATE parent_child_relationships 
SET new_child_id = m.new_profile_id
FROM profile_id_mapping m
WHERE parent_child_relationships.child_id = m.old_user_id;
...
ALTER TABLE parent_child_relationships RENAME COLUMN new_parent_id TO parent_id;
ALTER TABLE parent_child_relationships RENAME COLUMN new_child_id TO child_id;

-- Make the columns NOT NULL
ALTER TABLE parent_child_relationships ALTER COLUMN parent_id SET NOT NULL;
ALTER TABLE parent_child_relationships ALTER COLUMN child_id SET NOT NULL;
...
-- Add new column
ALTER TABLE user_progress ADD COLUMN IF NOT EXISTS new_user_id uuid;

-- Migrate existing data
UPDATE user_progress 
SET new_user_id = m.new_profile_id
FROM profile_id_mapping m
WHERE user_progress.user_id = m.old_user_id;

-- Drop old column and rename
ALTER TABLE user_progress DROP COLUMN user_id;
ALTER TABLE user_progress RENAME COLUMN new_user_id TO user_id;
...
-- Add new column
ALTER TABLE user_adventures ADD COLUMN IF NOT EXISTS new_user_id uuid;

-- Migrate existing data
UPDATE user_adventures 
SET new_user_id = m.new_profile_id
FROM profile_id_mapping m
WHERE user_adventures.user_id = m.old_user_id;

-- Handle any records without matches
UPDATE user_adventures SET new_user_id = gen_random_uuid() WHERE new_user_id IS NULL;

-- Drop old column and rename
ALTER TABLE user_adventures DROP COLUMN user_id;
ALTER TABLE user_adventures RENAME COLUMN new_user_id TO user_id;

-- Make the new column NOT NULL
ALTER TABLE user_adventures ALTER COLUMN user_id SET NOT NULL;
...
-- Add new column
ALTER TABLE user_path_progress ADD COLUMN IF NOT EXISTS new_user_id uuid;

-- Migrate existing data
UPDATE user_path_progress 
SET new_user_id = m.new_profile_id
FROM profile_id_mapping m
WHERE user_path_progress.user_id = m.old_user_id;

-- Handle any records without matches
UPDATE user_path_progress SET new_user_id = gen_random_uuid() WHERE new_user_id IS NULL;

-- Drop old column and rename
ALTER TABLE user_path_progress DROP COLUMN user_id;
ALTER TABLE user_path_progress RENAME COLUMN new_user_id TO user_id;

-- Make the new column NOT NULL
ALTER TABLE user_path_progress ALTER COLUMN user_id SET NOT NULL;
...
-- Add new column
ALTER TABLE user_rewards ADD COLUMN IF NOT EXISTS new_user_id uuid;

-- Migrate existing data
UPDATE user_rewards 
SET new_user_id = m.new_profile_id
FROM profile_id_mapping m
WHERE user_rewards.user_id = m.old_user_id;

-- Handle any records without matches
UPDATE user_rewards SET new_user_id = gen_random_uuid() WHERE new_user_id IS NULL;

-- Drop old column and rename
ALTER TABLE user_rewards DROP COLUMN user_id;
ALTER TABLE user_rewards RENAME COLUMN new_user_id TO user_id;

-- Make the new column NOT NULL
ALTER TABLE user_rewards ALTER COLUMN user_id SET NOT NULL;
...
-- Add new column
ALTER TABLE exercise_sessions ADD COLUMN IF NOT EXISTS new_user_id uuid;

-- Migrate existing data
UPDATE exercise_sessions 
SET new_user_id = m.new_profile_id
FROM profile_id_mapping m
WHERE exercise_sessions.user_id = m.old_user_id;

-- Handle any records without matches
UPDATE exercise_sessions SET new_user_id = gen_random_uuid() WHERE new_user_id IS NULL;

-- Drop old column and rename
ALTER TABLE exercise_sessions DROP COLUMN user_id;
ALTER TABLE exercise_sessions RENAME COLUMN new_user_id TO user_id;

-- Make the new column NOT NULL
ALTER TABLE exercise_sessions ALTER COLUMN user_id SET NOT NULL;

-- Add foreign key constraint
ALTER TABLE exercise_sessions ADD CONSTRAINT exercise_sessions_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES profiles(profile_id) ON DELETE CASCADE;

-- Step 18: Update unique constraints
...
-- Step 19: Create helper function for profile access
...
-- Step 20: Enable RLS on all tables
...
-- Step 21: Create new RLS policies for the restructured schema
...
CREATE POLICY "Children can view relationships"
  ...
  USING (
    EXISTS (
      SELECT 1 
      FROM profiles 
      WHERE profile_id = parent_child_relationships.child_id 
        AND user_id = auth.uid()
    )
  );
...
BEGIN
    ...
    RAISE NOTICE '   Children can be created without auth.users entries';
    RAISE NOTICE '   Parents maintain full oversight through RLS policies';
    RAISE NOTICE '   Complete data isolation between families preserved';
END $$;