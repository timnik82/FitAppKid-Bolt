/*
# Add Multilingual Support to Children's Fitness App

This migration adds comprehensive Russian language support to the database schema while maintaining full English support.

## Changes Made

1. **Adventure Content Localization**
   - Added `title_ru`, `description_ru`, `story_theme_ru` to adventures table
   - Added `title_ru`, `description_ru`, `theme_ru` to adventure_paths table
   - Added `title_ru`, `description_ru` to rewards table

2. **User Language Preferences**
   - Added `preferred_language` column to profiles table with EN/RU options

3. **Localization Helper Functions**
   - Created `get_localized_text()` function for text fallback logic
   - Created localized views for all major content tables

4. **Sample Russian Translations**
   - Added Russian translations for common categories, muscle groups, and equipment

5. **Documentation**
   - Added comprehensive column comments for multilingual fields
*/
