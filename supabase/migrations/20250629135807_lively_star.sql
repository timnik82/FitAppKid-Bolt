...
This migration completely resolves the infinite recursion error in RLS policies
by eliminating circular references and creating safe access patterns.

## Key Changes:
1. Create helper function in public schema (not auth schema)
2. Remove all recursive references in profiles table policies
3. Use direct subqueries instead of JOINs to profiles table
4. Implement clean separation between user access and parent-child access
...
CREATE POLICY "Parents can read children profiles"
  ...
  USING (
    ...
    AND profile_id IN (
      ...
      WHERE pcr.parent_id = public.get_current_user_profile_id()
        AND pcr.active = true
    )
  );
...
CREATE POLICY "Parents can create relationships"
  ...
  FOR INSERT
  TO authenticated
  WITH CHECK (parent_id = public.get_current_user_profile_id());
...
CREATE POLICY "Parents can read their relationships"
  ...
  FOR SELECT
  TO authenticated
  USING (parent_id = public.get_current_user_profile_id());
...
CREATE POLICY "Children can view their relationships"
  ...
  FOR SELECT
  TO authenticated
  USING (child_id = public.get_current_user_profile_id());
...
BEGIN
    ...
    RAISE NOTICE '   • Created public.get_current_user_profile_id() function';
    ...
    RAISE NOTICE '   • Other tables: Use public.get_current_user_profile_id() safely';
    RAISE NOTICE '';
